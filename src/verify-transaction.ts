...
import { Environment, accountB, accountA } from './constants';
...
const api = new rippleLib.RippleAPI({server: environment.testWssServer});
...
api.connect().then(() => {
  ...
  return api.prepareOrder(accountA.address, myOrder, myInstructions);
}).then(prepared => {
  console.log('Order Prepared');
  return api.getLedger().then(ledger => {
    console.log('Current Ledger', ledger.ledgerVersion);
    return submitTransaction(ledger.ledgerVersion, prepared, accountA.secret);
  });
}).then(() => {
  api.disconnect().then(() => {
